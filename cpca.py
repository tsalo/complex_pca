...
def write_results(pca_output, pca_type, mask, file_format, header, rotate, out_prefix):
    ...
    if file_format in ("nifti", "cifti"):
        if pca_type == "complex":
            ...
            write_out(
                np.abs(loadings), mask, header, file_format, f"{out_prefix}_magnitude"
            )
            write_out(
                np.real(loadings), mask, header, file_format, f"{out_prefix}_real"
            )
            write_out(
                np.imag(loadings), mask, header, file_format, f"{out_prefix}_imag"
            )
            write_out(
                np.angle(loadings), mask, header, file_format, f"{out_prefix}_phase"
            )
        elif pca_type == "real":
            write_out(loadings, mask, header, file_format, out_prefix)
...
def run_cpca(
    ...
    # load dataset
    if file_format == "txt":
        func_data = np.loadtxt(input_files).T
        if normalize == "zscore":
            func_data = zscore(func_data, nan_policy="omit")
    ...
    if recon & (pca_type == "complex"):
        ...
        cpca_recon(pca_output, rotate, file_format, mask, header, out_prefix, n_bins)
    ...
    write_results(pca_output, pca_type, mask, file_format, header, rotate, out_prefix)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f",
        "--file_format",
        help="the file format of the individual fMRI scans specified in input",
        required=False,
        default="nifti",
    ...
    parser.add_argument(
        "-b",
        "--bandpass_filter",
        help="Whether to bandpass filter time course w/ a butterworth filter",
        action="store_true",
        required=False,
    )
    parser.add_argument(
    ...
    run_cpca(
        input_files=args_dict["input"],
        n_comps=args_dict["n_comps"],
        mask_fp=args_dict["mask"],
        file_format=args_dict["file_format"],
        out_prefix=args_dict["output_prefix"],
        pca_type=args_dict["pca_type"],
        rotate=args_dict["rotate"],
        recon=args_dict["recon"],
        normalize=args_dict["normalize"],
        bandpass=args_dict["bandpass_filter"],
        low_cut=args_dict["bandpass_filter_low"],
        high_cut=args_dict["bandpass_filter_high"],
        tr=args_dict["sampling_unit"],
        n_bins=args_dict["n_recon_bins"],
        verbose=args_dict["verbose_off"],
    )
