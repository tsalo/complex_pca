...
def load_data(
    ...
    data, mask, header = load_scans(
        fps, file_format, mask_fp, normalize, bandpass, low_cut, high_cut, tr, verbose
    )
...
def load_scans(
    ...
    group_data = initialize_matrix(fps, n_scans, file_format, mask_bin, verbose)
    ...
    for fp in fps:
        # load file
        data, header = load_file(
            fp, file_format, mask_bin, bandpass, low_cut, high_cut, tr, verbose
        )
        # get # observations
        data_n = data.shape[0]
...
def load_file(fp, file_format, mask, bandpass, low_cut, high_cut, tr, verbose):
...
def parameter_check(fps, file_format, tr, bandpass, mask):
    ...
    if (mask is None) and (file_format == "nifti"):
        raise Exception("a mask .nii file must be supplied when file format is nifti")
...
def write_out(data, mask, header, file_format, out_prefix):
    ...
    if file_format == "nifti":
        ...
        nifti_4d = np.zeros(mask.shape + (data.shape[0],), dtype=data.dtype)
